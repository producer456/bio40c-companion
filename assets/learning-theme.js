...
export function timeline(state,connection,full=false) {
  ...
  const stale=!canvas?.lastSuccess||Date.now()/1000-canvas.lastSuccess>(connection?.public?7200:1800)||canvas.error;
...
export function nextActions(state,connection,now=Date.now()) {
  ...
  const fresh=canvas?.lastSuccess&&now/1000-canvas.lastSuccess<=(connection?.public?7200:1800)&&!canvas.error;
  ...
  return `<section class="card next-actions"><h2>What do I need to do next?</h2><p role="status">${connection?.public?'Shared class calendar':connection?.local.enabled?'Connected':'Not connected to Canvas'} · ${canvas?.lastSuccess?'Canvas checked '+esc(new Date(canvas.lastSuccess*1000).toLocaleString()):'Canvas has not been checked'}${fresh?'':' · Refresh needed'}</p>${canvas?.error||(!canvas&&connection?.message)?`<p class="notice">${esc(canvas?.error||connection.message)}</p>`:''}<p class="muted">${connection?.public?"Dates come from the site owner’s section; confirm your own deadlines. ":""}Calendar deadlines plus work you add. Undated work and class announcements may be missing. Marking done here does not submit to Canvas.</p>${connection?.local.enabled?`<button id="agenda-refresh" class="quiet">${connection?.public?'Refresh published dates':'Refresh deadlines'}</button>`:'<a href="#settings">Connect your private Canvas service</a>'}<h3>Next up</h3>${next?card(next,true):'<p>No unfinished work saved here. Check Canvas and add anything assigned in class.</p>'}<details class="class-capture"><summary>Add something from class</summary><form id="class-capture"><label>What needs doing?<input name="title" required maxlength="200" placeholder="Finish the lab worksheet"></label><label>Due date (optional)<input name="due" type="date"></label><label>Category<select name="category">${state.rules.map(r=>`<option value="${esc(r.id)}">${esc(r.name)}</option>`).join('')}</select></label><label>Next action (optional)<textarea name="note" maxlength="2000" rows="2" placeholder="Finish questions 4–8, then upload"></textarea></label><button type="submit">Save class assignment</button><p>Saved as your own assignment; confirm the deadline in Canvas.</p></form></details></section><section class="card due-soon"><h2>Due soon</h2>${['Overdue','Today','Tomorrow','This week','Confirm deadline'].map(group=>{const rows=items.filter(i=>i.group===group&&i.id!==next?.id);return rows.length?`<section><h3>${group}</h3>${rows.map(i=>card(i)).join('')}</section>`:'';}).join('')||'<p>No other saved work due soon.</p>'}<p><a href="#assignments">All deadlines & completed work →</a></p></section>`;
}
export function bindAgenda(getState,save,render,connection) {
  const current=typeof getState==='function'?getState:()=>getState;
  const update=async fn=>{try{const next=structuredClone(current());next.learningHub??={};next.learningHub.work??={};fn(next);validateHub(next);await save(next);render();}catch{/* The shared save handler displays errors and retains entries. */}};
...
export function bindHub(course,getState,save,render){
  const current=typeof getState==='function'?getState:()=>getState;
  async function update(fn,redraw=true){try{const next=structuredClone(current());next.learningHub??={};next.learningHub.work??={};next.learningHub.reviews??={};next.learningHub.questions??={};fn(next.learningHub);validateHub(next);await save(next);if(redraw)render();}catch{/* Shared save handler retains edits and reports failure. */}}
