...
function diagramPanel(source,stop,answer) {
  ...
  return `<details class="diagram-practice" data-diagram="${stop.id}"><summary>Diagram practice · ${esc(diagram.title)}</summary><button type="button" class="diagram-zoom" data-diagram-zoom aria-label="Enlarge diagram" title="Enlarge diagram" aria-pressed="false">+</button><div class="diagram-scroll" tabindex="0" aria-label="Original diagram with lettered label covers"><figure class="diagram-stage"><img src="./study-pages/${source}-p${diagram.page}.png" alt="${esc(diagram.title)}. Lettered positions correspond to the response fields below.">${diagram.labels.map((label,i)=>`<span class="diagram-mask" style="left:${label.box[0]}%;top:${label.box[1]}%;width:${label.box[2]}%;height:${label.box[3]}%" ${saved.revealed?'hidden':''}>${String.fromCharCode(65+i)}</span>`).join('')}</figure></div>${diagram.labels.map((label,i)=>`<label>${esc(label.prompt)}<input name="diagram-${stop.id}-${i}" maxlength="300" value="${esc(saved.answers[i]??'')}"></label>`).join('')}<button type="button" data-diagram-toggle="${stop.id}" aria-expanded="${saved.revealed}">${saved.revealed?'Cover labels':'Reveal original labels'}</button><div class="diagram-key" ${saved.revealed?'':'hidden'}>${diagram.labels.map((label,i)=>`<p><strong>${String.fromCharCode(65+i)}.</strong> ${esc(label.answer)}</p>`).join('')}</div></details>`;
}
function reviewPanel(course,state,native) {
  ...
  if(!reviewActive)return `<section class="study-review"><h2>Review today</h2><p>${queue.length?`${queue.length} checkpoints ready to revisit.`:'No checkpoints due. Study a lecture or lab to build your review list.'}</p>${queue.slice(0,5).map(({session,stop,reason})=>`<div class="review-row"><div><strong>${esc(stop.title)}</strong><p>${esc(session.title)} · ${esc(reason)}</p></div><button data-review-open="${session.source}/${stop.id}">Review</button></div>`).join('')}</section>`;
